...
if (!$hide_block && ($title || $description || $image)): ?>
  <section class="accordion_block fade-in relative" data-component="AccordionBlock">
    <div class="container-fluid">
      ...
      <div class="section-heading md:text-center !max-w-max">
        <?php if ($title): ?>
      ...
      <div class="flex flex-col gap-1 md:gap-3 items-center lg:items-stretch md:flex-row bg-sky-50 custom-rounded">
        <?php if ($image || ($accordion_items && !empty($accordion_items))): ?>
          <div class="w-full md:w-5/12 image-wrapper mx-auto bottom-right" data-accordion-image-container>
            <?php
        ...
        <?php if ($accordion_items): ?>
          <div class="w-full md:w-7/12 accordion_items-grid  p-4 md:p-8 relative overflow-hidden">
            <?php $i = 0;
            foreach ($accordion_items as $item): $i++;
              $is_open = $i === 1; ?>
              <div class="accordion_items-item border-b border-grey-7/20 last:border-b-0">
                <div class="accordion-trigger w-full flex items-center justify-between gap-4 py-5 md:py-6"
                  ...
                  <span class="icon-wrap shrink-0 inline-flex items-center justify-center rounded-full bg-red-600 text-white size-8 lg:size-8">
