    ...
    <div class="container-fluid xl:px-24 lg:px-14 px-5">
            ...
            ?>
              <div class="product-items products tab-item group relative w-full h-[100px] lg:max-h-none lg:w-[221px] lg:h-[400px] rounded-[20px] md:rounded-[24px] lg:rounded-[16px] overflow-hidden cursor-pointer transition-[max-height,transform,opacity] lg:transition-[width,transform,opacity] duration-700 ease-in-out bg-[#eaf3ff] shadow-[inset_0_0_1px_rgba(0,0,0,0.03)] will-change-[transform,opacity] lg:p-0  lg:[&.active]:max-h-none lg:[&.active]:w-[60vw]<?= $active_class; ?>" role="button" aria-expanded="<?= $active_class ? 'true' : 'false'; ?>">
                ...
                <div class="closed-state relative lg:absolute lg:inset-0 z-20 flex items-center lg:justify-start lg:items-center p-6 lg:p-4 size-full">
                  ...
                  <?php if ($item_title): ?>
                    <div class=" max-lg:flex max-lg:h-full max-lg:items-center title relative z-10 text-white font-medium md:font-semibold text-[18px] max-lg:leading-[20px] lg:text-[22px] tracking-[-0.48px] keep-all">
                      <?= esc_html($item_title); ?>
                    </div>
                  <?php endif; ?>
                </div>
                ...
                <div class="open-state hidden absolute inset-0 z-30 flex items-center justify-center p-5 lg:p-7">
                  <div class="content-card rounded-xl max-w-md lg:max-w-lg w-full">
                    <div class="content-wrapper space-y-2.5 md:space-y-3.5">
                      <?php if ($item_title): ?>
                        <div class="text-[18px] md:text-2xl font-medium md:font-semibold text-[#222]"><?= esc_html($item_title); ?></div>
                      <?php endif; ?>
                      <?php if ($item_content): ?>
                        <div class="description text-sm md:text-base font-normal"><?= wp_kses_post($item_content); ?></div>
                      <?php endif; ?>
                    </div>
                  </div>
                  <div class="curve-shape absolute  will-change-transform origin-bottom-right"></div>
                </div>
      ...
      <?php if ($button_link): ?>
        <div class="flex justify-center pt-4">
          <a href="<?= esc_url($button_link['url']); ?>" target="<?= esc_attr($cta_target); ?>" class="inline-flex items-center px-6 py-3 rounded-full bg-primary text-white font-semibold transition hover:brightness-110"><?= esc_html($button_link['title']); ?></a>
        </div>
      <?php endif; ?>
